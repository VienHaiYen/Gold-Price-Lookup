...
def reachAPI():
    ...
    res=res[2:]
    data=json.loads(res)
    with open("./server/data.json", "r", encoding='utf-8') as fin:
        file = json.load(fin)
        fin.close()
    file['last']=str(now)
    append=True
...
def deleteonlinelist():
    f=open('./server/onlinelist.json','r')
    data=json.load(f)
    f.close()
    data['account']=[]
    f=open('./server/onlinelist.json','w')
    f.write(json.dumps(data))
    f.close()
def onClosing():
    for con in onlineConn:
        con.sendall(bytes("SHUTDOWN",FORMAT))
    deleteonlinelist()
    my_window.destroy()
my_window =Tk()
my_window.title('Server Version')
my_window.geometry("550x350")
my_window.protocol("WM_DELETE_WINDOW", onClosing)
load=Image.open('./server/marketing.jpg')
...
def pageOne():
    ...
    tempIMG=(Image.open("./server/111.jpg"))
    ...
    with open('./server/infoclient.json', 'r') as f:
        data=json.load(f)
        f.close()
...
def pageTwo():
    ...
    with open('./server/onlinelist.json', 'r') as f:
        data=json.load(f)
        f.close()
        print(data)
    accounts=data['account']
...
def checkLogin(client):
    ...
    f=open('./server/infoclient.json','r')
    data=json.load(f)
    f.close()
    msg='declined'
...
def checkSignin(client):
    ...
    if msg=='signin succeed':
        ...
        data["account"].append(x)
        f=open('./server/infoclient.json','w+')
        f.write(json.dumps(data))
        f.close()
    print(msg)
def search(client):
